function data=P_bounding_energy(q_0,u_0)
%%
...
hold on
 for i =1:5
   ...
   data(5*i-4).energy = (1/2)*m.*(q(:,4).^2+q(:,5).^2) + m*g.*q(:,2) + (1/2)*I*q(:,6).^2;
   ...
   x_btoe = q_1(1) - L*cos(q_1(3)) + l_0*sin(u_0(1));
    ...
    data(5*i-3).energy = (1/2)*m.*(q(:,4).^2+q(:,5).^2) + m*g.*q(:,2) + (1/2)*I*q(:,6).^2;
    ...
    if events_back_stance == 1 %(front leg touchdown)    % proceed back leg stance phase to double stance phase
      ...
      data(5*i-2).energy = (1/2)*m.*(q(:,4).^2+q(:,5).^2) + m*g.*q(:,2) + (1/2)*I*q(:,6).^2;
        ...
        if events_double_stance == 1 %(back leg liftoff)   % proceed double stance phase to front leg stance phase
         ...
         time =time - t3(end);
             ...
             data(5*i-1).energy = (1/2)*m.*(q(:,4).^2+q(:,5).^2) + m*g.*q(:,2) + (1/2)*I*q(:,6).^2;
              ...
              time =time - t4(end);
                 ...
                 data(5*i).energy = (1/2)*m.*(q(:,4).^2+q(:,5).^2) + m*g.*q(:,2) + (1/2)*I*q(:,6).^2;
 ...
 xlabel('Time (s)');
 ylabel('Mechanical Engergy (J)');
 title('Bounding Gait with double stance from apex to next apex');
 legend('flight','back stance','double stance','front stance','flight');
 end
