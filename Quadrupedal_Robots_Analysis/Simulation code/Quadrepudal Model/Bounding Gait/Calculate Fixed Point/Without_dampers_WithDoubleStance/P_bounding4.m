function q_5=P_bounding4(q_0,u_0)
%%
...
%% 
  ...
  %q_1 =[x       ,y       ,theta   ,xdot    ,ydot    ,thetadot]
   time =time - t(end);
   x_btoe = q_1(1) - L*cos(q_1(3)) + l_0*sin(u_0(1));
    ...
    if events_back_stance == 1 %(front leg touchdown)    % proceed back leg stance phase to double stance phase
        ...
        [~,q,~,~,events_double_stance] =ode45(@double_stance_motion, [0,time] ,[q_2(1),q_2(2),q_2(3),q_2(4),q_2(5),q_2(6)] , options_double_stance);
        ...
        if events_double_stance == 1 %(back leg liftoff)   % proceed double stance phase to front leg stance phase
         ...
         time =time - t(end);
              ...
              [~,q,~,~,events_front_stance] =ode45(@front_stance_motion, [0,time] , q_3 , options_front_stance);
              ...
              time =time - t(end);
                  [~,q,~,~,~] =ode45(@flight_motion, [0,time] , q_4 , options_flight_2);
             
                   q = [q(end,1),q(end,2),q(end,3),q(end,4),q(end,5),q(end,6)];
                  q_5=[q(end,2),q(end,3),q(end,4),q(end,6)];
